...
;;;
;;(defun settings_for_client ()
;;  (interactive)
;;  (setq default-frame-alist '(
;;	(font-backend . "xft") 
;;	(set-background-color "#101416") 
;;	(set-foreground-color "#f6f3e8") 
;;	(font . "-unknown-DejaVu SansMono-normal-normal-normal-*-13-*-*-*-m-0-iso10646-1")
;;	))
;;)
;;
...
;;add timestamp, M-x my-insert-date
...
;;"find /usr/local/src/harrison/src_spamd/trunk /usr/include /usr/local/include -type d \( -name '*branches*' -o -name '*tag*' -o -name '*\.ori' -o -name '*\.svn' -o -name '*tmp' -o -name '*temp' \) -prune -o \( -name '*.c' -o -name '*.cpp' -o -name '*.h' \) -print > cscope.files
...
;;
;;
(if (bound-and-true-p cscope-mode)
...
;;             (define-key cscope:map "\C-sg" 'cscope-find-global-definition)
...
(add-to-list 'load-path "~/.emacs.d/elpa/auto-complete-20160107.8")
...
;;Begin of semantic ============================================================
;; Load CEDET
;;(load-file "~/.emacs.d/pkgs/cedet-1.0pre4/common/cedet.el")
;;Load for CEDET
(setq cedet-root-path
    (file-name-as-directory (expand-file-name
        "~/.emacs.d/pkgs/cedet-bzr/")))
(add-to-list 'load-path "~/.emacs.d/pkgs/cedet-bzr")
;;(add-to-list 'Info-directory-list
;;        "~/projects/cedet-bzr/doc/info")
;;(load "~/.emacs.d/pkgs/cedet-bzr/lisp/cedet/cedet.el")
(load "~/.emacs.d/pkgs/cedet-bzr/cedet-devel-load.el")
...
(global-ede-mode 1)
(require 'cedet)
(load "~/.emacs.d/pkgs/cedet-bzr/contrib/cedet-contrib-load.el")
...
(setq w3m-home-page "http://search.cpan.org/")
...
;;
;;;;;(set-default 'tramp-default-proxies-alist (quote ((".*" "\\'harrison.teng\\'" "/ssh:%h:"))))
...
;;Begin of multi-term ====================================================
(add-to-list 'load-path "~/.emacs.d/elpa/multi-term-20150220.520/")
...
;;;(set-default 'tramp-default-proxies-alist (quote ((".*" "\\'harrison.teng\\'" "/ssh2:%h:"))))
(set-default 'tramp-default-proxies-alist (quote ((".*" "\\'harrison.teng\\'" "/ssh2:bastion3ai.electric.net:"))))
;;;(add-to-list 'tramp-default-proxies-alist '(".*" "\'root\'" "/ssh2:%h:"))
(defun sudo-edit (&optional arg) 
    "Edit currently visited file as root.With a prefix ARG prompt for a file to visit.
  Will also prompt for a file to visit if current
  buffer is not visiting a file."
...
;;golang localdoc
(defun golangdoc-local()
  (interactive)
  (message (thing-at-point 'word))
  (godoc (thing-at-point 'word))
  )
...
(defun run-current-go-with-cmdopt ( Cmd )
 ;; "Run command "
   (interactive "sCmd: ")
   (shell-command Cmd)
)
...
(add-hook 'go-mode-hook
    (function (lambda ()
              (go-eldoc-setup)
              (my-go-mode-hook)
              (local-set-key (kbd "C-g C-w") 'golangdoc-webinfo-command)
              (local-set-key (kbd "C-g C-p") 'golangdoc-local)
              (local-set-key (kbd "C-g C-r") 'go-remove-unused-imports)
              (local-set-key (kbd "C-g C-i") 'go-goto-imports)
              (local-set-key (kbd "C-g C-j") 'godef-jump)
              (local-set-key (kbd "C-g C-d") 'godef-describe)
              (local-set-key (kbd "C-g C-r") 'run-current-go-with-cmdopt)
              (local-set-key (kbd "C-g C-c") 'go-errcheck)
)))

;;End of golang mode ========

;;(add-to-list 'load-path "~/.emacs.d/pkgs/cedet-1.0pre4/speedbar")
