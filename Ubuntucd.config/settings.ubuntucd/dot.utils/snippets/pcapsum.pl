...
use Switch;
 ...
 my $UNKNOWN_ERROR     = -1;
 my $PROGRAM_NAME      = 'pcapsum';
 my $PROGRAM_AUTHOR    = 'John Kristoff <jtk@cymru.com>';
 my $PROGRAM_URL       = 'http://www.cymru.com/jtk/code/';
 my ($PROGRAM_VERSION) = '$Revision: 1.5 $' =~ m{ [\$] Revision: \s+ (\S+) }xms;
...
sub parse_ip {
    ...
    my $payload = $pkt->{data};
    print "$pkt->{src_ip},$pkt->{dest_ip},$pkt->{proto},";
    $pcap_stat{ip}{pkts}++;
...
sub parse_tcp {
    ...
    my $payload   = $pkt->{data};
    print "$pkt->{src_port},$pkt->{dest_port},$payload\n";
