...
sub check_soft ($) {
        ...
        while (<RAID>) {
                ...
                if ( $devid >= 0 and /(.*?)blocks(.*)\[(.*?)\]$/ ) {
                        ...
                        $devid = -1 if ( $status eq 'DEGRADED' );
                        $raid = '' if ( $status eq 'DEGRADED' );
                        $status = '' if ( $status eq 'DEGRADED' );
                }
				
				if ( $devid >= 0 and /(.*?)resync =(.*)/) {
						$status = 'DEGRADED';
                        $stats{"md$devid"} = "is $raid and its status is $status";
                        $devid = -1;
                        $raid = '';
				...
				#md1 : active raid10 sdb2[1] sda2[0] sdd2[3] sdc2[2]
      			#488018944 blocks super 1.2 512K chunks 2 near-copies [4/4] [UUUU]
      			#[>....................]  resync =  1.0% (4967104/488018944) finish=272.4min speed=29546K/sec
      			#
				...
				#
				if ( /\[(.*?)\]\s+(resync|recovery)/ ){
					(my $resync = $_ ) =~ s/\[(.*)\]\s+(resync|recovery)\s=\s+(.*?)\%(.*)/$3/;
					($stats{'rebuild'} = $resync) =~ s/\s//g;
				}
        ...
        #alertemail( "Only $total MDs are found, expected $options{'total'} on $hostname","contact $techemail immediately ( less disks )",CRITICAL ) if $total < $options{'total'};
        alert( "Only $total MDs are found, expected $options{'total'} on $hostname",CRITICAL ) if $total < $options{'total'};

        #if any MDs are DEGRADED
        #alertemail( 'There are DEGRADED ones found in : '.join(',',values(%stats)),"contact $techemail immediately",CRITICAL ) if grep /DEGRADED/,values( %stats );
        if ( ( grep /DEGRADED/,values( %stats ) ) ) {
			my $msg = "DEGRADED ones are : \n".join(",\n",map{ $_.' '.$stats{$_} } grep ( /md/, keys %stats ) );
			#if any md is rebuilding ..
			#alertemail( 'Raid is rebuilding...','Raid is rebuilding'.$stats{'rebuild'}.'%',CRITICAL ) if ( $stats{'rebuild'} > 0 );
			if ( $stats{'rebuild'} > 0 ) {
				$msg = 'Raid is rebuilding ( '.$stats{'rebuild'}.'% ) '.$msg;	
			}
        	alert( $msg,CRITICAL );
		}
...
sub check_mega ($) {
	...
	if ( $verbose ) {
        ...
        #alertemail( "Only $total PDs are found, expected $options{'total'} on $hostname","contact $techemail immediately",CRITICAL ) if $total < $options{'total'};
        alert( "Only $total PDs are found, expected $options{'total'} on $hostname",CRITICAL ) if $total < $options{'total'};

        #if any PDs are DEGRADED
        #alertemail( 'There are DEGRADED ones : '.$raidmsg,"contact $techemail immediately",CRITICAL ) unless ( grep /Optimal/i,@vdState );
        alert( 'There are DEGRADED ones : '.$raidmsg,CRITICAL ) unless ( grep /Optimal/i,@vdState );
...
sub check_areca ($) {
	...
	if ( $raidsetstatus ne 'Normal' or $volumesetstatus ne 'Normal' ) {
		...
		#alertemail( 'Areca Raid is '.$raidsetstatus,$summary,$alertcode );
		alert( $summary,$alertcode );
	}
	...
	if ( $diskfound != $diskexpected or $diskfound != $options{'total'} ) {
		...
		#alertemail( 'Areca Raid got '.$diskfound.' disks,'.$diskexpected.' is expected !',$summary,$alertcode );
		alert( $summary,$alertcode );
	}
...
MAIN: {
        ...
        if ( $options{'model'} eq 'md' ) {
                ...
                unless ( -f $statfile ) {
                        #alertemail( "No $statfile is located on $hostname","contact $techemail immediately",CRITICAL );
                        alert( "No $statfile is located on $hostname",CRITICAL );
                }else{
                        check_soft( $statfile );
                }
